...
public class Marker implements Comparable<Marker> {
    ...
    private static final String TAG="MarkerOutside";
    ...
    public Marker(String name, double latitude, double longitude, double altitude, int color) {
        ...
        mPaintScreen = new PaintScreen(Color.GRAY);
    }
    ...
    public void populateMatrices(CameraModel cam, float addX, float addY) {
        if (cam == null)
            throw new NullPointerException();
        ...
        Log.d(TAG, "Start");
        Log.d(TAG, String.format("-- 1. MTemp : %s", tmpLocationVector));
        tmpLocationVector.add(locationXyzRelativeToPhysicalLocation);
        Log.d(TAG, String.format("-- 2. MTemp : %s", tmpLocationVector));
        tmpLocationVector.prod(ARData.getRotationMatrix());
        Log.d(TAG, String.format("-- 3. MTemp : %s", tmpLocationVector));
        cam.projectPoint(tmpLocationVector, tmpVector, addX, addY);
        Log.d(TAG, String.format("-- 4. Result : %s", tmpVector));
        Log.d(TAG, "Finish");
...
//    }
    ...
    public void calcRelativePosition(Location location) {
        ...
        Log.d("outside.Marker", "Location relative to location : " + locationXyzRelativeToPhysicalLocation);
        Log.d("outside.Marker", "Updated distance : " + distance);
        //updateRadar();
    }
...
//            drawPosition(canvas);
        ...
        mPaintScreen.circle(canvas, x, y, 25);
    }
    ...
    private void drawPosition(Canvas canvas) {
        ...
        Log.d("Marker", String.format("Screen position : x %.4f y %.4f", x, y));
