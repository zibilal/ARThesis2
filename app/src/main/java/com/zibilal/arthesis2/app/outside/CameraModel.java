...
public class CameraModel {
    ...
    public void projectPoint(Vector orgPoint, Vector prjPoint, float addX, float addY) {
        orgPoint.get(tmp1);
        Log.d(TAG, String.format("\n\n-- Width : %d Height : %d ViewAngle : %.4f Distance : %.4f", width, height, viewAngle, distance));
        Log.d(TAG, String.format("-- Input point = %.4f %.4f %.4f", tmp1[0], tmp1[1], tmp1[2]));
        tmp2[0] = (distance * tmp1[0] / -tmp1[2]);
        tmp2[1] = (distance * tmp1[1] / -tmp1[2]);
        tmp2[2] = (tmp1[2]);
        tmp2[0] = (tmp2[0] + addX + width / 2);
        tmp2[1] = (-tmp2[1] + addY + height / 2);
        Log.d(TAG, String.format("-- Output point = %.4f %.4f %.4f", tmp2[0], tmp2[1], tmp2[2]));
        prjPoint.set(tmp2);
    }
